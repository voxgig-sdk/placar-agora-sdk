...
local PlacarAgoraSDK = {}
PlacarAgoraSDK.__index = PlacarAgoraSDK
...
PlacarAgoraSDK._make_feature = _make_feature


function PlacarAgoraSDK.new(options)
  local self = setmetatable({}, PlacarAgoraSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function PlacarAgoraSDK:options_map()
...
function PlacarAgoraSDK:get_utility()
  return Utility.copy(self._utility)
end


function PlacarAgoraSDK:get_root_ctx()
  return self._rootctx
end


function PlacarAgoraSDK:prepare(fetchargs)
...
function PlacarAgoraSDK:direct(fetchargs)
...
-- by passing it.
function PlacarAgoraSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function PlacarAgoraSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "PlacarAgoraSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function PlacarAgoraSDK:Score(data)
  ...
  if data == nil then
    if self._score == nil then
      self._score = EntityMod.new(self, nil)
    end
    return self._score
  end
  return EntityMod.new(self, data)
...
function PlacarAgoraSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = PlacarAgoraSDK.new(sdkopts)
  sdk.mode = "test"
