...
local ScoreEntity = {}
ScoreEntity.__index = ScoreEntity


function ScoreEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ScoreEntity)
  self._name = "score"
  self._client = client
...
function ScoreEntity:get_name()
  return self._name
end


function ScoreEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ScoreEntity.new(self._client, opts)
end
...
function ScoreEntity:mark_deleted()
  self._deleted = true
end


function ScoreEntity:deleted()
  return true == self._deleted
end


function ScoreEntity:data_set(args)
  if args ~= nil then
...
function ScoreEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ScoreEntity:match_set(args)
  if args ~= nil then
...
function ScoreEntity:match_get()
...
function ScoreEntity:stream(action, args, callopts)
...
function ScoreEntity:_run_op(ctx, post_done)
  local utility = self._utility
