...
{
    private static function utility(): PlacarAgoraUtility
    {
        return new PlacarAgoraUtility();
    }
    ...
    private static function ctx(array $over = []): PlacarAgoraContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new PlacarAgoraContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new PlacarAgoraOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof PlacarAgoraError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new PlacarAgoraOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = PlacarAgoraMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new PlacarAgoraOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = PlacarAgoraMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new PlacarAgoraOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = PlacarAgoraMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new PlacarAgoraSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = PlacarAgoraMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new PlacarAgoraResponse([]),
            'result' => new PlacarAgoraResult([]),
        ]);
        [, $err] = PlacarAgoraMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new PlacarAgoraSpec([]),
            'response' => null,
            'result' => new PlacarAgoraResult([]),
        ]);
        [, $err] = PlacarAgoraMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new PlacarAgoraSpec(['step' => 's']),
            'response' => new PlacarAgoraResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new PlacarAgoraResult([]),
        ]);
        [, $err] = PlacarAgoraMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new PlacarAgoraSpec(['step' => 's']),
            'response' => new PlacarAgoraResponse(self::resp(200, ['v' => 1])),
            'result' => new PlacarAgoraResult([]),
        ]);
        [, $err] = PlacarAgoraMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new PlacarAgoraSpec(['step' => 's']),
            'response' => new PlacarAgoraResponse(self::resp(200, ['v' => 2])),
            'result' => new PlacarAgoraResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        PlacarAgoraMakeResponse::call($ctx);
    ...
    {
        $preset = new PlacarAgoraResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new PlacarAgoraSpec([]),
            'response' => new PlacarAgoraResponse([]),
            'result' => new PlacarAgoraResult([]),
        ...
        [$got, $err] = PlacarAgoraMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new PlacarAgoraResult([])]);
        [, $err] = PlacarAgoraMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new PlacarAgoraSpec([]), 'result' => null]);
        [, $err] = PlacarAgoraMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new PlacarAgoraSpec(['step' => 's']),
            'result' => new PlacarAgoraResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new PlacarAgoraOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = PlacarAgoraMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new PlacarAgoraSpec(['step' => 's']),
            'result' => new PlacarAgoraResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new PlacarAgoraOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = PlacarAgoraMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new PlacarAgoraResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new PlacarAgoraSpec([]),
            'result' => new PlacarAgoraResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = PlacarAgoraMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = PlacarAgoraMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new PlacarAgoraError('boom', 'boom');
        $utility->fetcher = function (PlacarAgoraContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new PlacarAgoraSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = PlacarAgoraMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (PlacarAgoraContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = PlacarAgoraMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(PlacarAgoraResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        PlacarAgoraMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (PlacarAgoraContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new PlacarAgoraSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = PlacarAgoraMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new PlacarAgoraResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new PlacarAgoraSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = PlacarAgoraMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = PlacarAgoraMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = PlacarAgoraMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new PlacarAgoraResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, PlacarAgoraDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new PlacarAgoraResult(['ok' => false])]);
        $this->expectException(PlacarAgoraError::class);
        PlacarAgoraDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new PlacarAgoraResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', PlacarAgoraMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        PlacarAgoraMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new PlacarAgoraBaseFeature();
        $b = new PlacarAgoraBaseFeature();
        PlacarAgoraFeatureAdd::call($ctx, $a);
        PlacarAgoraFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): PlacarAgoraBaseFeature
    {
        $f = new PlacarAgoraBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        PlacarAgoraFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): PlacarAgoraContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new PlacarAgoraSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = PlacarAgoraPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        PlacarAgoraPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new PlacarAgoraResponse(['headers' => null]),
            'result' => new PlacarAgoraResult([]),
        ]);
        PlacarAgoraResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        PlacarAgoraResultBody::call($ctx);
