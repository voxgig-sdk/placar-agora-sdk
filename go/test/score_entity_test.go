...
func TestScoreEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Score(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Score(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		scoreRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.score")))
...
func scoreBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("PLACAR_AGORA_TEST_SCORE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["PLACAR_AGORA_TEST_SCORE_ENTID"])
