...
import (
	...
	omni "github.com/voxgig-sdk/placar-agora-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.PlacarAgoraError); is {
				out := map[string]any{
					"name":    "PlacarAgoraError",
...
func omniCtx(arg any, client *sdk.PlacarAgoraSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.PlacarAgoraSDK); is {
			client = live
