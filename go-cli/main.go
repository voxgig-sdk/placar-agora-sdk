// Command PlacarAgora-cli is a CLI + REPL for the PlacarAgora SDK,
...
const prompt = "placar-agora"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "schedule score"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      PLACAR_AGORA_APIKEY (api key), PLACAR_AGORA_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: PLACAR_AGORA_APIKEY carries the API key and
	// PLACAR_AGORA_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("PLACAR_AGORA_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("PLACAR_AGORA_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewPlacarAgoraSDK(opts)
