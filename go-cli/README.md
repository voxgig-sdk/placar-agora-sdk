...
boru-driven command-line client **and** interactive REPL for the PlacarAgora
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/placar-agora-cli)
make build

# 2. See usage (words, entities, env vars)
./placar-agora-cli --help

# 3. Provide credentials once, via the environment
export PLACAR_AGORA_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./placar-agora-cli list schedule
./placar-agora-cli list score

# 5. Override the API base URL for a single call
PLACAR_AGORA_BASE=https://api.example.com ./placar-agora-cli list schedule

# 6. No arguments -> interactive REPL
./placar-agora-cli
placar-agora> list schedule
placar-agora> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/placar-agora-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/placar-agora-cli list schedule
   ```
...
Run with no arguments to open a REPL (prompt `placar-agora>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/placar-agora-cli`. |
...
(`./placar-agora-cli <expr>`) and interactively (the REPL), and expressions compose the
