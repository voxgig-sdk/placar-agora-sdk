...
class ScoreDirectTest < Minitest::Test
  def test_direct_list_score
    setup = score_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-score", setup[:live] ? "live" : "unit")
    if _should_skip
...
def score_direct_setup(mockres)
