...
require_relative 'PlacarAgora_types'
...
class PlacarAgoraSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = PlacarAgoraHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = PlacarAgoraHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, PlacarAgoraFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    PlacarAgoraUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = PlacarAgoraHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = PlacarAgoraHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = PlacarAgoraHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = PlacarAgoraHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = PlacarAgoraError.new(
        "graphql_error", "PlacarAgoraSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = PlacarAgoraSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
