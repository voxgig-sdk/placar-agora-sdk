...
class ScoreEntity
  ...
  def make
    opts = @_entopts.dup
    ScoreEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = PlacarAgoraHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Score, Hash] the current Score data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = PlacarAgoraHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Score fields)
  def match_get
  ...
  #
  # @param reqmatch [ScoreListMatch, Hash, nil] match filter (any subset of
  #   Score fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Score>, Array] the matching Score items; raises PlacarAgoraError on failure
