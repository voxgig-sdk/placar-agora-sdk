...
module PlacarAgoraUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = PlacarAgoraResponse.new({})
    ...
    if fetch_err
      ...
      response = PlacarAgoraResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = PlacarAgoraResponse.new(fetched)
    else
