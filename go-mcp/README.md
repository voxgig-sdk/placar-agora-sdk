...
[MCP](https://modelcontextprotocol.io) server exposing the PlacarAgora SDK as
two agent tools — `placar-agora_list` and `placar-agora_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/placar-agora-mcp)
make build

# 2. Provide credentials via the environment
export PLACAR_AGORA_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user placar-agora \
  -- /absolute/path/to/placar-agora-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./placar-agora-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// placar-agora_list: first page of records
{ "entity": "schedule" }
{ "entity": "schedule", "query": { } }

// placar-agora_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/placar-agora-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user placar-agora \
     -- "$PWD"/dist/*/placar-agora-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `placar-agora_list` and `placar-agora_load` tools now appear
   in new sessions. Ask the agent to *"list schedule using placar-agora"*
   and it calls `placar-agora_list` with `{"entity":"schedule"}`.
...
```sh
export PLACAR_AGORA_APIKEY=sk_live_xxx            # API key
export PLACAR_AGORA_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `placar-agora_list` tool
...
### Call the `placar-agora_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `placar-agora_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `placar-agora_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `PLACAR_AGORA_APIKEY` | API key sent with every request. |
| `PLACAR_AGORA_BASE` | Optional override of the API base URL. |
...
```sh
./placar-agora-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"placar-agora_load","arguments":{"entity":"schedule","query":{"id":1}}}}'
```
