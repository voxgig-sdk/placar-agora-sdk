...
class PlacarAgoraTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            PlacarAgoraTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = PlacarAgoraTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = PlacarAgoraTestRunner.getenv("PLACAR_AGORA_TEST_LIVE")
        override = PlacarAgoraTestRunner.getenv("PLACAR_AGORA_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = PlacarAgoraTestRunner.getenv(key)
        ...
        explain = PlacarAgoraTestRunner.getenv("PLACAR_AGORA_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if PlacarAgoraTestRunner._test_control is not None:
            return PlacarAgoraTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                PlacarAgoraTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            PlacarAgoraTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = PlacarAgoraTestRunner.load_test_control()
...
def load_env_local():
    PlacarAgoraTestRunner.load_env_local()


def env_override(m):
    return PlacarAgoraTestRunner.env_override(m)


def entity_data(v):
    return PlacarAgoraTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return PlacarAgoraTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return PlacarAgoraTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return PlacarAgoraTestRunner.load_test_control()


def live_client_options():
    return PlacarAgoraTestRunner.live_client_options()


def live_delay_ms():
    return PlacarAgoraTestRunner.live_delay_ms()
