...
class TestScoreEntity:
    ...
    def test_should_stream(self):
        ...
        base = PlacarAgoraSDK.test(seed, None)
        seen = list(base.Score(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            sdk = PlacarAgoraSDK.test(
                seed, {"feature": {"streaming": {"active": True}}})
            got = []
            for item in sdk.Score(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _score_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["list"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "score." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set PLACAR_AGORA_TEST_SCORE_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        score_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.score")))
        score_ref01_data = None
        if len(score_ref01_data_raw) > 0:
...
def _score_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/score/ScoreTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("PLACAR_AGORA_TEST_SCORE_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("PLACAR_AGORA_TEST_LIVE") == "TRUE":
        merged_opts = vs.merge([
            {
            },
            extra or {},
        ])
        client = PlacarAgoraSDK(helpers.to_map(merged_opts))

    _live = env.get("PLACAR_AGORA_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("PLACAR_AGORA_TEST_EXPLAIN") == "TRUE",
        "live": _live,
