...
class TestScoreDirect:

    def test_should_direct_list_score(self):
        setup = _score_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-score", "live" if setup["live"] else "unit")
        if _skip:
...
def _score_direct_setup(mockres):
