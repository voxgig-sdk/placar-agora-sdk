...
from placaragora_sdk.placaragora_types import (
    Score,
    ScoreListMatch,
)
...
class ScoreEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return ScoreEntity(self._client, opts)
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Score]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Score().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
