...
//     (\`new PlacarAgoraSDK(...)\` / \`PlacarAgoraSDK.test(...)\` become
//     \`PlacarAgoraSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"schedule":{"test01":{"id":"test01"}},"score":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new PlacarAgoraSDK(...)\` and any
// \`PlacarAgoraSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+PlacarAgoraSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/PlacarAgoraSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
