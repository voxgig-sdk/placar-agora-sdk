...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.PLACARAGORA_TEST_LIVE ||
    'TRUE' === process.env.PLACARAGORA_TEST_OVERRIDE
  ) {
  ...
  m.PLACARAGORA_TEST_EXPLAIN = process.env.PLACARAGORA_TEST_EXPLAIN || m.PLACARAGORA_TEST_EXPLAIN
