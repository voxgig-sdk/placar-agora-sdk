...
class ScoreEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new ScoreEntity($this->_client, $opts);
    }

    /**
     * @param Score|array $args Score data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = PlacarAgoraHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Score|array The current Score data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Score fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = PlacarAgoraHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Score fields).
     */
    ...
    /**
     * List Score items matching the given filter.
     *
     * @param ScoreListMatch|array|null $reqmatch Match filter (any subset
     *   of Score fields) as an assoc-array; ScoreListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Score[]|array A list of Score items as assoc-arrays at
     *   the SDK boundary; throws PlacarAgoraError on failure (item-5 convention).
     */
