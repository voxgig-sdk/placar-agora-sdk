...
class FtCtrl extends PlacarAgoraControl
{
...
class FtClient
{
    ...
    private PlacarAgoraUtility $utility;

    public function __construct(PlacarAgoraUtility $utility, array $options)
    {
    ...
    public function get_utility(): PlacarAgoraUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (PlacarAgoraContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public PlacarAgoraContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (PlacarAgoraContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = PlacarAgoraConfig::make_feature($name);
    ...
    private static function buildUrl(PlacarAgoraSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof PlacarAgoraError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new PlacarAgoraResponse($fetched);
        }
    ...
    private function fail(PlacarAgoraContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (PlacarAgoraContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
